...
class ElectrumWatcher(val client: ElectrumClient, val scope: CoroutineScope, loggerFactory: LoggerFactory) : CoroutineScope by scope {
    ...
    init {
        ...
        fun startTimer() {
            ...
            val timeMillis: Long = 5L * 1_000 // fire timer every 5 seconds
            timerJob = launch {
                delay(timeMillis)
                while (isActive) {
                    ...
                    is WatcherCommand.ProcessConnectionStatus -> {
                        when (cmd.status) {
                            ...
                            is ElectrumConnectionStatus.Closed -> {
                                state = state.copy(height = 0, scriptHashSubscriptions = setOf(), scriptHashStatus = mapOf())
                                stopTimer()
                            }
                        }
                    }
                    ...
                    is WatcherCommand.ProcessNotification -> {
                        when (cmd.notification) {
                            ...
                            is HeaderSubscriptionResponse -> {
                                ...
                                state = state.copy(block2tx = state.block2tx - toPublish.keys, sent = state.sent + txs)
